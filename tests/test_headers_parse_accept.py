...
class ParseAcceptHeaderTests(unittest.TestCase):
    ...
    def test_that_all_items_are_returned(self) -> None:
        parsed = headers.parse_accept(
            'audio/*;q=0.2,audio/basic,audio/aiff;q=0'
        )
        self.assertEqual(len(parsed), 3)

    def test_that_highest_priority_is_first(self) -> None:
        parsed = headers.parse_accept(
            'audio/*;q=0.2,audio/basic,audio/aiff;q=0'
        )
        self.assertEqual(
            parsed[0], datastructures.ContentType('audio', 'basic')
        )

    def test_that_quality_parameter_is_removed(self) -> None:
        parsed = headers.parse_accept(
            'audio/*;q=0.2,audio/basic,audio/aiff;q=0'
        )
        for value in parsed:
    ...
    def test_that_specific_value_without_parameters_is_second(self) -> None:
        parsed = headers.parse_accept(
            'text/*, text/plain,text/plain;format=flowed, */*'
        )
        self.assertEqual(
    ...
    def test_that_subtype_wildcard_is_next_to_last(self) -> None:
        parsed = headers.parse_accept(
            'text/*, text/plain,text/plain;format=flowed, */*'
        )
        self.assertEqual(parsed[2], datastructures.ContentType('text', '*'))

    def test_that_least_specific_wildcard_is_least_preferred(self) -> None:
        parsed = headers.parse_accept(
            'text/*, text/plain,text/plain;format=flowed, */*'
        )
        self.assertEqual(parsed[3], datastructures.ContentType('*', '*'))

    def test_that_extension_tokens_are_parsed(self) -> None:
        self.assertEqual(
    ...
    def test_that_extension_tokens_with_spaces_are_parsed(self) -> None:
        self.assertEqual(
            headers.parse_accept('application/json;x-foo=" something else"'),
            [
                datastructures.ContentType(
                    'application', 'json', {'x-foo': ' something else'}
                ),
            ],
        )
...
class ParseAcceptCharsetHeaderTests(unittest.TestCase):
    ...
    def test_that_simple_wildcard_parses(self) -> None:
        self.assertEqual(headers.parse_accept_charset('*'), ['*'])

    def test_that_response_sorted_by_quality(self) -> None:
        self.assertEqual(
            headers.parse_accept_charset(
                'us-ascii;q=0.1, latin1;q=0.5,utf-8; q=1.0'
            ),
    ...
    def test_that_wildcard_sorts_before_rejected_character_sets(self) -> None:
        self.assertEqual(
            headers.parse_accept_charset(
                'latin1;q=0.5, utf-8;q=1.0,us-ascii;q=0.1, ebcdic;q=0, *'
            ),
            ['utf-8', 'latin1', 'us-ascii', '*', 'ebcdic'],
        )

    def test_that_quality_below_0_001_is_rejected(self) -> None:
        self.assertEqual(
    ...
    def test_that_wildcard_sorts_before_rejected_character_sets(self) -> None:
        self.assertEqual(
            headers.parse_accept_encoding(
                'gzip;q=0.5, compress;q=1.0,bzip;q=0.1, snappy;q=0, *'
            ),
...
class ParseAcceptLanguageTests(unittest.TestCase):
    ...
    def test_that_wildcard_sorts_before_rejected_character_sets(self) -> None:
        self.assertEqual(
            headers.parse_accept_language(
                'es;q=0.5, es-mx;q=1.0,es-es;q=0.1, es-pr;q=0, *'
            ),
    ...
    def test_that_order_is_retained_without_quality(self) -> None:
        self.assertEqual(
            headers.parse_accept_language(
    ...
    def test_that_explicit_highest_quality_is_first(self) -> None:
        self.assertEqual(
            headers.parse_accept_language(
